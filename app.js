...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (docSnap.exists()) { 
            ...
            fetchedUserData = { name: "Player", number: user.email.replace("@youthearners.com", ""), wallet: 0, xp: 0 }; 
        }
    } else { 
...
registerForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const userData = { uid: user.uid, name: name, number: number, wallet: 50, xp: 0 }; 
...
loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        let userData = { name: "Player", number: number, wallet: 0, xp: 0 };
        if (docSnap.exists()) { userData = docSnap.data(); }
        
        loginBtn.innerText = "Log In"; loginBtn.disabled = false;
        launchGame(userData);
    } catch (error) {
        alert("Login failed! Check number and password.");
        loginBtn.innerText = "Log In"; loginBtn.disabled = false;
    }
});
...
function launchGame(userData) {
    ...
    if(bgm) { bgm.volume = 0.2; bgm.play().catch(e => console.log("BGM pending user interaction")); }
